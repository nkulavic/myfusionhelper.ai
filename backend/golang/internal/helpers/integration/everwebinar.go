...
// EverWebinar registers a CRM contact for an EverWebinar session.
...
type EverWebinar struct{}

func (h *EverWebinar) GetName() string     { return "EverWebinar" }
func (h *EverWebinar) GetType() string     { return "everwebinar" }
func (h *EverWebinar) GetCategory() string { return "integration" }
func (h *EverWebinar) GetDescription() string {
	return "Registers a CRM contact for an EverWebinar session"
}
func (h *EverWebinar) RequiresCRM() bool       { return true }
func (h *EverWebinar) SupportedCRMs() []string { return nil }

func (h *EverWebinar) GetConfigSchema() map[string]interface{} {
	return map[string]interface{}{
		"type": "object",
		"properties": map[string]interface{}{
			"webinar_id": map[string]interface{}{
				"type":        "string",
				"description": "The EverWebinar webinar ID",
			},
			...
			"service_connection_ids": map[string]interface{}{
				"type": "object",
				"properties": map[string]interface{}{
					"everwebinar": map[string]interface{}{
						"type":        "string",
						"description": "The service connection ID for EverWebinar authentication",
					},
				},
...
func (h *EverWebinar) ValidateConfig(config map[string]interface{}) error {
...
func (h *EverWebinar) Execute(ctx context.Context, input helpers.HelperInput) (*helpers.HelperOutput, error) {
	...
	// Get service auth for EverWebinar
	auth := input.ServiceAuths["everwebinar"]
	if auth == nil {
		output.Message = "EverWebinar service connection is not configured"
		return output, fmt.Errorf("everwebinar service auth not found")
	}

	apiKey := auth.APIKey
	if apiKey == "" {
		output.Message = "EverWebinar API key is missing"
		return output, fmt.Errorf("everwebinar api key not found")
	}
	...
	if email == "" {
		output.Message = "Contact email is required for EverWebinar registration"
		return output, fmt.Errorf("contact email is empty")
	}

	// Build the EverWebinar registration payload
	...
	// Build the API URL
	apiURL := "https://api.webinarjam.com/everwebinar/register"

	output.Success = true
	output.Message = fmt.Sprintf("EverWebinar registration prepared for %s (webinar: %s)", email, webinarID)
	...
	output.Logs = append(output.Logs, fmt.Sprintf("EverWebinar registration for contact %s (%s) to webinar %s", input.ContactID, email, webinarID))
