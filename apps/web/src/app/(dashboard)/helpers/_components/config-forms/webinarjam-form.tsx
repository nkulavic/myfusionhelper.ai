...
// Schema: see schemas.ts > webinarJamSchema
import { TagPicker } from '@/components/tag-picker'
import { FormTextField } from './form-fields'
import type { ConfigFormProps } from './types'

export function WebinarJamForm({ config, onChange, disabled, platformId, connectionId }: ConfigFormProps) {
  ...
  return (
    <div className="space-y-4">
      <FormTextField
        label="Webinar ID"
        placeholder="Enter your WebinarJam ID"
        value={webinarId}
        onChange={(e) => updateConfig({ webinar_id: e.target.value })}
        disabled={disabled}
        description="The unique ID for your WebinarJam live webinar."
        required
      />
